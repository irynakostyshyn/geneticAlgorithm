...
void Individual::update_values(long double &step, double &lower_bound) {
    values.clear();
    values = vector<double>(chromosomes.size() / chromosome_length);
    int number;
    int power;
    int current_ind = 0;
    for (unsigned int i = 0, j = chromosome_length;
         j <= chromosomes.size(); j += chromosome_length, i += chromosome_length) {
        number = 0;
        power = 0;
        ...
        // changed to real values
        values[current_ind] = lower_bound + (number * step);
        current_ind++;
    };
}
...
void Individual::flip(unsigned long i) {
    chromosomes[i] = 1 - (int) chromosomes[i];
}
...
void Individual::print() {
    ...
    cout << std::endl << "Fitness value: " << fitness_value << endl;
}
